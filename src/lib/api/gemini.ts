...
/** 429 / 503 時に順に試すモデル。2.5-flash → 1.5-flash の順でフォールバック */
 ...
 * それでもダメなら次モデルへフォールバック（gemini-2.5-flash → gemini-1.5-flash）。
 */
async function generateContentWithFallback(apiKey: string, prompt: string): Promise<string> {
  ...
  for (const modelId of GEMINI_MODELS) {
    for (let attempt = 0; attempt < maxRetriesPerModel; attempt++) {
      try {
        ...
        const combined = `${lastError.message} ${lastDetail}`
        const isRetryable =
        ...
        if (canRetry) {
          ...
          const ms = waitSec != null ? Math.ceil(waitSec * 1000) + 800 : 5_000
  ...
  console.error('[Gemini] 全モデルで失敗', { models: [...GEMINI_MODELS], lastDetail })
  throw new Error(
...
※「---」の下には推敲後本文のみを記述してください。タイトル・監修者ブロックのテキストは含めないでください。
