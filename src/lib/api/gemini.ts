...
- 本文中でセクション区切りとして「---」（水平線）を使用しないこと。「---」は冒頭のタイトル行と本文の区切りとして1回だけ使われる特殊記号であり、本文内部で複数回使ってはならない
...
参照資料に明記された数値以外は「ある中小企業」「ある製造業の経営者」程度の粒度にする。`.trim()
  ...
  if (!/タイトル[:：]/.test(text) && text.indexOf('---') === -1) {
    throw new Error('生成結果の形式が不正です。再度お試しください。')
  }

  const parsed = parseGeneratedArticleText(text)
...
※本文に ** や * を含めないこと（上記ルールの再確認）`.trim()
  ...
  let refinedContent = parsed.content
